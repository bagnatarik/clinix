  ...
  template: `
    <form [formGroup]="form" (ngSubmit)="submit()" class="space-y-6">
      <div class="bg-white border border-neutral-300 rounded p-6">
        <h2 class="text-xl font-bold text-gray-800">Attribuer chambre</h2>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
          ...
          <div>
            ...
            <input type="text" class="w-full border border-neutral-300 rounded px-3 py-2 text-sm" formControlName="chambre" placeholder="Ex: A-201" />
          </div>
          <div>
            <label class="block text-xs font-medium text-gray-500 mb-1">Service</label>
            <input type="text" class="w-full border border-neutral-300 rounded px-3 py-2 text-sm" formControlName="service" placeholder="Service / Département" />
          </div>
          <div>
            <label class="block text-xs font-medium text-gray-500 mb-1">Date d’admission</label>
            <input type="date" class="w-full border border-neutral-300 rounded px-3 py-2 text-sm" formControlName="admissionDate" />
          </div>
        </div>
      </div>
      ...
      <div class="flex gap-2">
        ...
        <a class="px-5 py-2.5 border border-gray-600 text-gray-600 rounded hover:bg-gray-600 hover:text-white transition hover:cursor-pointer text-sm" [routerLink]="['/dashboard/infirmier/hospitalisations/list']">Annuler</a>
      </div>
    </form>
  `,
})
