    ...
    <form [formGroup]="form" (ngSubmit)="submit()" class="space-y-6">
      <div class="bg-white border border-neutral-300 rounded p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
          ...
          <div>
            <label class="block text-xs font-medium text-gray-500 mb-1">Traitement</label>
            <input type="text" class="w-full border border-neutral-300 rounded px-3 py-2 text-sm" formControlName="traitement" placeholder="Ex: Antibiotique" />
          </div>
          <div>
            <label class="block text-xs font-medium text-gray-500 mb-1">Date de début</label>
            <input type="date" class="w-full border border-neutral-300 rounded px-3 py-2 text-sm" formControlName="debut" />
          </div>
          <div>
      ...
      <div class="flex gap-2">
        ...
        <a class="px-5 py-2.5 border border-gray-600 text-gray-600 rounded hover:bg-gray-600 hover:text-white transition hover:cursor-pointer text-sm" [routerLink]="['/dashboard/infirmier/traitements/list']">Annuler</a>
      </div>
    </form>
  `,
})
export class TraitementsNewComponent {
  ...
  constructor(private fb: FormBuilder, private router: Router) {
    this.form = this.fb.group({
      ...
      debut: [this.today(), Validators.required],
